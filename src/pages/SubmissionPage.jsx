/**
 * Submission confirmation page component.
 *
 * Triggers submission via {@link AccountContext} on mount (with idempotency
 * check to prevent duplicate submissions). Renders the
 * {@link SubmissionConfirmation} component with the submission results.
 *
 * Protected route requiring full auth chain (authenticated, verified,
 * and token validated). Final step in the card management workflow.
 *
 * Uses {@link AccountContext} for submission state and results.
...
import { SubmissionConfirmation } from '../components/review/SubmissionConfirmation.jsx';
...
/**
 * Checkmark SVG icon for the submission page branding area.
 *
 ...
 * Submission confirmation page component.
 *
 * Triggers submission via AccountContext on mount with an idempotency
 * check to prevent duplicate submissions. Renders the submission
 * confirmation component with the results. Protected route requiring
 * full auth chain — redirects unauthenticated users to login, unverified
 * users to verification, and users without valid tokens to token validation.
 *
 * @param {Object} [props]
 * @param {function} [props.onDone] - Callback invoked when the user clicks
 *   the "Done" button. Typically navigates to the starting state or dashboard.
 * @param {function} [props.onStartNew] - Callback invoked when the user clicks
 *   the "Start New" button to begin a new signer management workflow.
 ...
 * @returns {React.ReactElement} The rendered submission page.
 */
export function SubmissionPage({
  ...
  /**
   * Triggers submission on mount if there are staged changes and no
   * submission result yet. Uses a ref to prevent duplicate submissions
   * across re-renders.
   */
  useEffect(() => {
    if (
      ...
      try {
        ...
        // Submission errors are handled by AccountContext and displayed via the component.
      }
    }
  }, [
    authLoading,
      ...
      <AppLayout>
        ...
        >
          ...
          >
            ...
            >
              You must be logged in to access the submission confirmation page. Please sign in with your credentials to continue.
            </p>
      ...
      <AppLayout>
        ...
        >
          ...
          >
            ...
            >
              Your identity must be verified before you can access the submission confirmation page. Please complete the verification process to continue.
            </p>
      ...
      <AppLayout>
        ...
        >
          ...
          >
            ...
            >
              Your eSign token must be validated before you can access the submission confirmation page. Please complete the token validation process to continue.
            </p>
        ...
        >
          ...
          >
            ...
            <Alert
              ...
              message="Please select a business banking account before viewing submission confirmation."
            />
  ...
  // Submission in progress — show loading spinner
  if (accountLoading && !submissionResult) {
    return (
      <AppLayout>
        <div
          className={wrapperClassName}
        ...
        >
          <LoadingSpinner size="large" label="Submitting changes…" />
          <p
            style={{
              fontSize: 'var(--hb-font-size-sm)',
              color: 'var(--hb-secondary-gray)',
          ...
          >
            Please wait while your changes are being submitted. Do not close or refresh this page.
          </p>
        </div>
      </AppLayout>
    );
  ...
  // No submission result and no staged changes — nothing to show
  if (!submissionResult && (!stagedChanges || !stagedChanges.hasChanges)) {
    return (
      <AppLayout>
        <div
          className={wrapperClassName}
  ...
  // Fully authenticated, verified, token validated, account selected — show submission confirmation
  return (
    <AppLayout>
      <div
        className={wrapperClassName}
